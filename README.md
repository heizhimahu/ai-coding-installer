# AI Coding Installer v1.2.0

为校园学生远程装机场景设计的 Claude Code + OpenClaw 三档服务套餐安装工具。

**版本**: 1.2.0，含三档套餐菜单 / dry-run / check-only，不含 GUI / 套餐系统 / 自动更新。
...
| 进阶工作流包 | ¥98 | Claude Code **或** OpenClaw (二选一) | 新手教程 + 5 套指定工作流教程 | 14 天 |
| 全套效率包 | ¥158/¥198 | Claude Code **+** OpenClaw (两个都装) | 新手教程 + 10 套指定工作流教程 | 14 天 |

### 套餐适用人群

- **基础上手包 (¥58)** — 刚接触 AI 编程助手，想体验基本功能。只需一个工具入门，预算敏感。
- **进阶工作流包 (¥98)** — 已有编程基础，想快速掌握 5 套高效工作流。单工具深度使用场景。
- **全套效率包 (¥158/¥198)** — 想同时使用两个工具互补，配合 10 套工作流打通完整开发链路。适合深度用户或团队。
...
- 如果 Node.js 版本低于 22.16 且用户选择安装 OpenClaw，脚本会显示警告但不会强制升级。
...
报告记录：套餐名称、价格、售后天数、选择安装的软件、需交付的工作流数量、每步执行结果。

## 更新安装命令

所有安装命令集中在脚本顶部的变量区：

- `install-mac-linux.sh` 第 49-75 行
- `install-windows.ps1` 第 28-40 行
...
- 不创建任何后门、隐藏进程或后台常驻服务
- 不上传日志到任何服务器
- OpenClaw 默认使用 no-onboard 模式安装，用户本人运行 onboarding
